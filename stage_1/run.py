...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if extract_flow:
            ...
            pts0 = render_result2['pts_o'].cpu().numpy()
            pts1 = render_result2['pts_delta'].cpu().numpy()
            ...
            for i2 in range(uv0.shape[0]):
                if(depth[uv0[i2,1],uv0[i2,0]] > 100.0) and (uv1[i2,0]>0 and uv1[i2,0]< W-1) and (uv1[i2,1]>0 and uv1[i2,1]< H-1):
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict):
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        loss += 0.05 * cfg_train.weight_main * F.mse_loss(render_result['rgb_marched2'], target) #0.07 0.003 0.001
        ...
        vel_loss = torch.zeros_like(loss)   
        ...
        ghost_loss = torch.zeros_like(loss)  
        if cfg_train.weight_ghost > 0:
            render_color = render_result['rgb_marched']
            bkg_color = torch.ones_like(render_color) if cfg.data.white_bkgd else torch.zeros_like(render_color)
